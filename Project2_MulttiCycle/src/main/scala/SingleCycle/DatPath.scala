...
//---------------next pc unit-------------//
//----------------------------------------//
	val br_exten= Cat(Fill(15, io.ctl.Imm16(15)), io.ctl.Imm16(14, 0), Fill(2, io.ctl.Imm16(15)))
	...
	PCafter := pc_reg + 4.U(32.W)
	pc_br := pc_reg + br_exten + 4.U(32.W)

	val pc4_en = Mux(io.ctl.ALUctr === "b011".U(3.W), false.B,
				 true.B)

	pc_next := MuxCase(PCafter, Array(
						(pc4_en === true.B && 
						 io.ctl.nPC_sel === false.B)	 -> PCafter,
